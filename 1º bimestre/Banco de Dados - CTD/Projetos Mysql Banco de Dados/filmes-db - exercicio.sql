...
-- Quantos filmes estão registrados?
SELECT F.id, COUNT(F.titulo) 
FROM filmes AS F;

-- Quantos filmes têm entre 3 e 7 prêmios?

SELECT F.titulo, F.premios, F.avaliacao
FROM filmes AS F
WHERE F.premios BETWEEN 3 AND 7
ORDER BY F.premios ASC; 
